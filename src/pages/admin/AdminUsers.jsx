...
import { Plus, Pencil, Trash2, Search, ToggleLeft, ToggleRight, RefreshCw } from 'lucide-react';
...
const Users = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await usersApi.getAll(page, itemsPerPage, searchTerm || null, roleFilter === 'all' ? null : roleFilter);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage system users and their roles ({total} users)</p>
        </div>
                      ...
                      <TableCell>
                        ...
                        >
                          <SelectTrigger className="w-32 h-8">
                            <span className={`px-3 py-1 rounded-md text-xs font-medium ${getRoleBadgeClass(user.role)}`}>
                              {user.role}
                            </span>
                          </SelectTrigger>
                          <SelectContent>
